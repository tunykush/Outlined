...
const MONTH_LOOKUP = Object.fromEntries(MONTHS.flatMap((m, i) => [
    [m.toLowerCase(), m],
...
export function parseDateParts(raw) {
    ...
    if (m)
        return { year: m[1], month: normaliseMonth(m[2]), day: normaliseDay(m[3]) };
    m = s.match(/^(\d{1,2})[-/](\d{1,2})[-/](\d{4}[a-z]?)$/i);
    if (m)
        return { year: m[3], month: normaliseMonth(m[2]), day: normaliseDay(m[1]) };
    m = s.match(/^(\d{1,2})(?:st|nd|rd|th)?\s+([A-Za-z]+)\s+(\d{4}[a-z]?)$/i);
    if (m)
        return { year: m[3], month: normaliseMonth(m[2]), day: normaliseDay(m[1]) };
    m = s.match(/^([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?\s+(\d{4}[a-z]?)$/i);
    if (m)
        return { year: m[3], month: normaliseMonth(m[1]), day: normaliseDay(m[2]) };
...
export function dayMonthYear(raw) {
...
export function monthDayYear(raw) {
    const p = parseDateParts(raw);
    if (!p.year && !p.month && !p.day)
        return clean(raw);
    if (p.month && p.day && p.year)
...
function titleFromDomain(host) {
    if (!host)
        return '';
    if (SPECIAL_SITES[host])
        return SPECIAL_SITES[host];
 ...
 */
function cleanCompoundSiteName(siteName, url) {
...
function splitName(raw) {
    ...
    if (orgRe.test(s) || s.split(/\s+/).length >= 5)
...
function normaliseAuthor(a) {
    ...
    if (/^(openai|anthropic|google|microsoft|rmit)$/i.test(out.family))
        out.isOrganisation = true;
    return out;
}
...
function inferAiCreator(tool) {
    const t = clean(tool).toLowerCase();
    if (/claude|anthropic/.test(t))
        return 'Anthropic';
    if (/gemini|google/.test(t))
        return 'Google';
    if (/copilot|microsoft|bing/.test(t))
        return 'Microsoft';
    if (/chatgpt|openai|gpt/.test(t))
        return 'OpenAI';
    return 'OpenAI';
}
function normaliseReportNumber(s) {
    const v = clean(s);
    if (!v)
        return '';
    return /^\(.+\)$/.test(v) ? v.slice(1, -1) : v;
}
export function normalizeCitationData(style, source, input) {
    ...
    if (d.year && !/^\d{4}[a-z]?$/i.test(d.year) && d.year !== 'n.d.') {
        ...
        if (p.year) {
            d.year = p.year;
            d.month || (d.month = p.month);
            d.day || (d.day = p.day);
        }
    }
    const host = hostname(d.url);
    if (isGenericSiteName(d.siteName) && host)
        d.siteName = titleFromDomain(host);
    if (isGenericSiteName(d.publisher) && host)
        d.publisher = titleFromDomain(host);
    if (!d.siteName && host)
        d.siteName = titleFromDomain(host);
    ...
    if (d.siteName)
        d.siteName = cleanCompoundSiteName(d.siteName, d.url);
    if (d.publisher)
        d.publisher = cleanCompoundSiteName(d.publisher, d.url);
    // DOI beats URL for journal articles in both uploaded style guides.
    if (source === 'journal' && d.doi)
        d.url = d.url && /^https?:\/\/(?:dx\.)?doi\.org\//i.test(d.url) ? '' : d.url;
    ...
    if (source === 'ai-chat') {
        if (!d.toolName)
            d.toolName = /openai/i.test(d.authors[0]?.family || '') ? 'ChatGPT' : clean(d.platform || 'AI tool');
        if (!d.title && style === 'apa7')
            d.title = 'Untitled AI output';
    }
    if (source === 'youtube-video' && !d.platform)
        d.platform = 'YouTube';
    if (source === 'social-twitter' && !d.platform)
        d.platform = 'X';
    if (source === 'social-instagram' && !d.platform)
        d.platform = 'Instagram';
    if (source === 'social-facebook' && !d.platform)
        d.platform = 'Facebook';
    if (source === 'social-tiktok' && !d.platform)
        d.platform = 'TikTok';
    return d;
}
