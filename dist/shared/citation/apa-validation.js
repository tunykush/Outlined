...
function validAuthors(d) { return (d.authors || []).some((a) => has(a.family) || has(a.given)); }
function hasLocator(d) { return has(d.quotePage) || has(d.quotePages) || has(d.quoteSection) || has(d.quoteParagraph) || has(d.timestamp); }
const REQUIRED = {
    webpage: ['title', 'url'],
...
const DIRECT_QUOTE_SOURCE_TYPES = new Set([
...
function repairDoiUrl(reference) {
    return reference
...
function removeFinalFullStopAfterUrl(reference) {
    return reference.trim().replace(/(https?:\/\/\S+)\.$/i, '$1');
}
function requiredFieldNotes(source, data) {
    const notes = [];
    for (const field of REQUIRED[source] || []) {
        if (!has(String(data[field] || '')))
            notes.push(`APA validator warning: missing ${String(field)} for ${source}.`);
    }
...
function confidence(source, data) {
    ...
    if (source === 'journal' && !has(data.doi) && !has(data.url))
        score -= 10;
    if ((source === 'webpage' || source === 'blog-post' || source === 'newspaper-online') && !has(data.siteName) && !has(data.publisher))
        score -= 10;
    return Math.max(20, Math.min(100, score));
}
export function validateAndRepairApa7(source, data, output) {
