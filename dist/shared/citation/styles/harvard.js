...
const endFullStop = (s) => /[.!?]$/.test(stripHtml(s).trim()) ? s.trim() : `${s.trim()}.`;
const joinNonEmpty = (parts, sep = ', ') => parts.filter((p) => has(stripHtml(p))).join(sep);
const noFinalPeriodAfterUrl = (s) => s.trim().replace(/(https?:\/\/\S+)\.$/i, '$1');
const MONTHS = [
    'January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December',
];
const MONTH_LOOKUP = Object.fromEntries(MONTHS.flatMap((m, i) => [[m.toLowerCase(), m], [m.slice(0, 3).toLowerCase(), m], [String(i + 1), m], [String(i + 1).padStart(2, '0'), m]]));
function initialsNoStops(given) {
    return clean(given)
        .split(/\s+/)
        .filter(Boolean)
...
function peopleTextHarvard(authors = [], raw = '') {
...
function inTextAuthor(authors = []) {
    ...
    if (list.length === 1)
        return name(list[0]);
    if (list.length === 2)
        return `${name(list[0])} and ${name(list[1])}`;
...
function parseLooseDate(raw) {
    ...
    if (m)
        return { day: String(Number(m[3])), month: normaliseMonth(m[2]), year: m[1] };
    m = s.match(/^(\d{1,2})[-/](\d{1,2})[-/](\d{4})$/);
    if (m)
        return { day: String(Number(m[1])), month: normaliseMonth(m[2]), year: m[3] };
    m = s.match(/^(\d{1,2})(?:st|nd|rd|th)?\s+([A-Za-z]+)\s+(\d{4}[a-z]?)$/i);
    if (m)
        return { day: normaliseDay(m[1]), month: normaliseMonth(m[2]), year: m[3] };
    m = s.match(/^([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?\s+(\d{4}[a-z]?)$/i);
    if (m)
        return { day: normaliseDay(m[2]), month: normaliseMonth(m[1]), year: m[3] };
    m = s.match(/^(\d{4}[a-z]?)$/i);
    if (m)
        return { day: '', month: '', year: m[1] };
    return { day: '', month: '', year: '' };
}
function dayMonthYear(day, month, year) {
...
function sourceDateText(d) {
    return dayMonthYear(d.day, d.month, yearOnly(d));
}
function accessDateText(d) {
    const fromRaw = parseLooseDate(d.accessDate);
    if (fromRaw.year || fromRaw.month || fromRaw.day)
        return dayMonthYear(fromRaw.day, fromRaw.month, fromRaw.year);
...
function edn(edition) {
    const e = clean(edition);
    if (!e || /^1(st)?(?:\s+edn?)?$/i.test(e))
        return '';
    if (/edn$/i.test(e))
        return e;
    if (/ed\.?$/i.test(e))
        return e.replace(/ed\.?$/i, 'edn');
    if (/\d+(st|nd|rd|th)$/i.test(e))
        return `${e} edn`;
    return `${e} edn`;
}
function websiteName(d) {
...
function refLead(d, titleHtmlForNoAuthor, date = dateYear(d)) {
    ...
    if (author)
        return { lead: `${esc(author)} ${date} `, omittedTitle: false };
    return { lead: `${titleHtmlForNoAuthor} ${date} `, omittedTitle: true };
}
function titleSnippet(d, words = 4) {
    return clean(d.title).split(/\s+/).slice(0, words).join(' ') || 'Title';
}
const ITALIC_NO_AUTHOR_IN_TEXT = new Set([
    'webpage',
    'webpage-document',
    'wiki-entry',
    'book',
...
function noAuthorText(d, source) {
    if (source === 'journal')
        return clean(d.journal || d.title || 'Title');
    if (source === 'newspaper-online' || source === 'newspaper-print')
        return clean(d.publisher || d.siteName || d.title || 'Title');
    if (source === 'ai-chat')
        return clean(d.publisher || d.toolName || d.title || 'OpenAI');
    const snippet = esc(titleSnippet(d));
    return ITALIC_NO_AUTHOR_IN_TEXT.has(source) ? ital(snippet) : `'${snippet}'`;
}
function quoteLocatorHarvard(d, source) {
    ...
    if (time)
        return time;
    if (section && para)
        return `${section} section, para. ${para}`;
    if (section)
        return `${section} section`;
    if (para)
        return `para. ${para}`;
    if (source === 'social-twitter' ||
...
function harvardInTextParenthetical(d, source) {
    if (source === 'legal-act')
        return `(${esc(clean(d.title) || 'Title of Act')} ${esc(yearOnly(d))}${has(d.section) ? ` s ${esc(clean(d.section))}` : ''})`;
    if (source === 'legal-case')
        return `(${ital(esc(clean(d.title) || 'Case title'))} ${esc(yearOnly(d))})`;
    if (source === 'personal-communication')
        return harvardPersonalCommunicationParenthetical(d);
...
function harvardInTextNarrative(d, source) {
    if (source === 'personal-communication')
        return harvardPersonalCommunicationNarrative(d);
    const author = inTextAuthor(d.authors);
    if ((source === 'newspaper-online' || source === 'newspaper-print') && !author) {
        return `${esc(noAuthorText(d, source))} (${esc(sourceDateText(d))})`;
    }
...
function harvardInTextQuote(d, source) {
    if (source === 'personal-communication')
        return harvardPersonalCommunicationParenthetical(d);
...
function articleNumberPart(raw) {
    const n = clean(raw);
    if (!n)
        return '';
    return /^article\b/i.test(n) ? n : `Article ${n}`;
}
function journalLocator(d) {
...
function harvardJournal(d) {
    ...
    if (vol)
        out += `, ${esc(vol)}${issue ? `(${esc(issue)})` : ''}`;
    if (locator)
        out += `:${esc(locator)}`;
    if (has(d.doi))
        out += `, ${doiHarvard(d.doi)}.`;
    else if (has(d.url))
        out += `, ${websiteName(d) || 'website'}, ${accessPart(d)}. ${clean(d.url)}`;
    else
        out += '.';
    return noFinalPeriodAfterUrl(out);
}
function harvardBook(d) {
    const title = ital(esc(clean(d.title) || 'Untitled book'));
    const { lead, omittedTitle } = refLead(d, title);
    const parts = [omittedTitle ? lead.trim() : `${lead.trim()} ${title}`];
    const edition = edn(d.edition);
    if (edition)
        parts.push(esc(edition));
    if (has(d.publisher))
        parts.push(esc(clean(d.publisher)));
    if (has(d.doi))
        parts.push(doiHarvard(d.doi));
    else
        parts.push(esc(clean(d.place || 'n.p.')));
    return endFullStop(joinNonEmpty(parts));
}
function harvardTranslatedBook(d) {
    const title = ital(esc(clean(d.title) || 'Untitled book'));
    const { lead, omittedTitle } = refLead(d, title);
    const parts = [omittedTitle ? lead.trim() : `${lead.trim()} ${title}`];
    if (has(d.translatorsText))
        parts.push(`translated by ${esc(clean(d.translatorsText))}`);
    const edition = edn(d.edition);
    if (edition)
        parts.push(esc(edition));
    if (has(d.publisher))
    ...
    if (has(d.originalYear))
        parts.push(`original work published ${esc(clean(d.originalYear))}`);
    return endFullStop(joinNonEmpty(parts));
}
function harvardBookChapter(d) {
    const author = authorsHarvard(d.authors);
...
function harvardSocial(d, defaultPostType) {
...
function harvardFilm(d) {
    ...
    if (has(d.seriesTitle))
        parts.push(esc(clean(d.seriesTitle)));
    if (companies)
        parts.push(esc(companies));
    if (has(d.place))
...
function harvardStreamingVideo(d) {
...
function harvardTvEpisode(d) {
    ...
    if (has(d.productionCompanies || d.publisher))
        parts.push(esc(clean(d.productionCompanies || d.publisher)));
    if (has(d.place))
        parts.push(esc(clean(d.place)));
    return endFullStop(joinNonEmpty(parts));
}
function harvardImage(d) {
    ...
    if (has(d.publisher || d.siteName))
        parts.push(esc(clean(d.publisher || d.siteName)));
    if (has(d.url)) {
        parts.push(accessPart(d));
        return appendUrl(joinNonEmpty(parts), d.url);
    }
...
function harvardCourseMaterial(d, defaultFormat) {
...
function harvardThesis(d) {
    ...
    if (has(d.institution))
        parts.push(esc(clean(d.institution)));
    if (has(d.repository))
        parts.push(`${esc(clean(d.repository))} database`);
    if (has(d.url)) {
        parts.push(accessPart(d));
        return appendUrl(joinNonEmpty(parts), d.url);
    }
...
function harvardPersonalCommunication(d) {
...
export function generateHarvard(source, data) {
    return {
