...
const dot = (s) => /[.!?]$/.test(stripHtml(s).trim()) ? s : `${s}.`;
const noFinalPeriodAfterUrl = (s) => s.replace(/\s+\./g, '.').trim();
...
function normalizeOrgCandidate(s) {
...
function splitName(raw) {
    ...
    if (!s)
        return { family: '', given: '' };
    if (normalizeOrgCandidate(s) || s.split(/\s+/).length >= 5)
...
function validPeople(authors = []) {
    return authors.filter((a) => has(a.family) || has(a.given));
}
function formatPersonAPA(a) {
    if (a.isOrganisation)
        return clean(a.family);
...
function authorsAPA(authors = []) {
    ...
    if (!list.length)
        return '';
    if (list.length === 1)
        return formatPersonAPA(list[0]);
    if (list.length === 2)
        return `${formatPersonAPA(list[0])}, & ${formatPersonAPA(list[1])}`;
    if (list.length <= 20) {
...
function authorsAPAFromText(raw) {
    return authorsAPA(parsePeople(raw));
}
function formatPersonInitialsFirst(a) {
    if (a.isOrganisation)
        return clean(a.family);
    const ini = initials(a.given);
    return ini ? `${ini} ${clean(a.family)}` : clean(a.family);
}
function contributorsInitialsFirst(authors = [], raw = '') {
...
function authorsWithRole(authors = [], role) {
    const auth = authorsAPA(authors);
    return auth ? `${auth} (${role})` : '';
}
function authorsInText(authors = [], narrative = false) {
    const list = validPeople(authors);
    if (!list.length)
        return '';
    const name = (a) => clean(a.family || a.given);
    if (list.length === 1)
        return name(list[0]);
    if (list.length === 2)
        return `${name(list[0])} ${narrative ? 'and' : '&'} ${name(list[1])}`;
    return `${name(list[0])} et al.`;
}
function apaDate(d, includeMonthDay) {
...
function authorOrTitleLead(d, date, titleForNoAuthor) {
    const auth = authorsAPA(d.authors);
    if (auth)
        return { lead: `${dot(esc(auth))} ${date} `, omitTitle: false, orgUsedAsAuthor: false };
    const orgName = clean(d.siteName || d.publisher || '');
    if (orgName)
        return { lead: `${dot(esc(orgName))} ${date} `, omitTitle: false, orgUsedAsAuthor: true };
...
function resolveInTextAuthor(d, narrative = false) {
...
const AV_TIMESTAMP_TYPES = new Set([
    'youtube-video',
    'film',
    'podcast',
    'streaming-video',
...
const NO_LOCATOR_QUOTE_TYPES = new Set([
    'social-twitter',
    'social-facebook',
    'social-instagram',
    'social-tiktok',
]);
function noAuthorInTextTitle(d, source) {
    const t = esc(titleSnippet(d));
    if (source === 'legal-act' || source === 'legal-case')
        return ital(esc(clean(d.title) || 'Title'));
    if (ITALIC_IN_TEXT_NO_AUTHOR.has(source))
        return ital(t);
    return `&quot;${t}&quot;`;
}
function quoteLocator(d, source) {
    if (NO_LOCATOR_QUOTE_TYPES.has(source))
        return '';
    ...
    if (time)
        return esc(time);
    if (section && para)
        return `${esc(section)} section, para. ${esc(para)}`;
    if (section)
        return `${esc(section)} section`;
    if (para)
        return `para. ${esc(para)}`;
    if (AV_TIMESTAMP_TYPES.has(source))
 ...
 * ============================================================ */
function apaWebpageLike(d, includeMonthDay) {
...
const apaWebpage = (d) => apaWebpageLike(d, Boolean(clean(d.month) || clean(d.day)));
const apaWebpageDocument = (d) => apaWebpageLike(d, false);
function apaWikiEntry(d) {
...
function apaNewspaperOnline(d) {
    ...
    if (!omitTitle)
        out += `${title}. `;
    if (has(d.publisher) || has(d.siteName))
        out += `${ital(esc(clean(d.publisher || d.siteName)))}.`;
    if (has(d.url))
        out += ` ${clean(d.url)}`;
    return noFinalPeriodAfterUrl(out);
}
function apaNewspaperPrint(d) {
    const date = apaDate(d, true);
    const title = esc(sentenceCase(d.title));
    const { lead, omitTitle } = authorOrTitleLead(d, date, title);
    let out = lead;
    ...
    if (has(d.publisher) || has(d.siteName))
        out += ital(esc(clean(d.publisher || d.siteName)));
    if (has(d.pages))
        out += `, ${esc(clean(d.pages))}`;
    return dot(out.trim());
}
function apaJournal(d) {
    ...
    if (!omitTitle)
        out += `${title}. `;
    if (has(d.journal))
        out += ital(esc(clean(d.journal)));
    if (has(d.volume))
        out += `${has(d.journal) ? ', ' : ''}${ital(esc(clean(d.volume)))}`;
    if (has(d.issue))
        out += `(${esc(clean(d.issue))})`;
    const locatorParts = [];
    if (has(d.pages))
        locatorParts.push(esc(clean(d.pages)));
...
function editionPart(edition) {
    const ed = clean(edition);
    if (!ed || /^1(st)?(?:\s+ed\.)?$/i.test(ed))
        return '';
    if (/ed\.?$/i.test(ed))
        return ed.replace(/\.?$/, '.');
    if (/\d+(st|nd|rd|th)$/i.test(ed))
        return `${ed} ed.`;
    return `${ed} ed.`;
}
function apaBook(d) {
...
function apaTranslatedBook(d) {
    ...
    if (has(d.publisher))
        out += `${esc(clean(d.publisher))}.`;
    if (has(d.originalYear))
        out += ` (Original work published ${esc(clean(d.originalYear))})`;
    if (has(d.doi))
        out += ` ${doiUrl(d.doi)}`;
    else if (has(d.url))
        out += ` ${clean(d.url)}`;
...
function apaReport(d) {
    const date = apaDate(d, false);
    let title = ital(esc(sentenceCase(d.title)));
    if (has(d.reportNumber))
        title += ` (${esc(clean(d.reportNumber))})`;
    const { lead, omitTitle } = authorOrTitleLead(d, date, title);
    const auth = authorsAPA(d.authors);
...
function apaBlogPost(d) {
    ...
    if (!omitTitle)
        out += `${title}. `;
    if (has(d.siteName))
        out += `${ital(esc(clean(d.siteName)))}. `;
    if (has(d.url))
        out += clean(d.url);
    return noFinalPeriodAfterUrl(out);
}
function apaSocial(d, platformDefault, descDefault, postTypeDefault) {
    const auth = authorsAPA(d.authors);
    const date = apaDate(d, true);
    ...
    let lead = '';
    if (auth)
        lead = `${dot(`${esc(auth)}${handle}`)} ${date} `;
    else
        lead = `${dot(ital(`${esc(title)}${ellipsis}`))} ${date} `;
    let out = lead;
    if (auth)
        out += `${ital(`${esc(title)}${ellipsis}`)} `;
    if (desc)
        out += `[${esc(desc)}] `;
...
function apaFilm(d) {
    ...
    if (auth)
        out += `${ital(esc(sentenceCase(d.title)))} [Film]. `;
    else
        out += `[Film]. `;
    if (has(d.productionCompanies || d.publisher))
        out += `${esc(clean(d.productionCompanies || d.publisher))}.`;
    return noFinalPeriodAfterUrl(out);
}
function apaPodcast(d) {
...
function apaStreamingVideo(d) {
    ...
    if (auth)
        out += `${title}. `;
    if (has(d.publisher || d.platform))
        out += `${esc(clean(d.publisher || d.platform))}. `;
    if (has(d.url))
        out += clean(d.url);
    return noFinalPeriodAfterUrl(out);
}
function apaTvSeries(d) {
    ...
    if (auth)
        out += `${ital(esc(sentenceCase(d.title)))} [TV series]. `;
    else
        out += `[TV series]. `;
    if (has(d.productionCompanies || d.publisher))
        out += `${esc(clean(d.productionCompanies || d.publisher))}.`;
...
function apaTvEpisode(d) {
    const writers = authorsAPAFromText(d.writersText);
    const directors = authorsAPAFromText(d.directorsText);
    const creators = [];
    if (writers)
        creators.push(`${writers} (Writer${parsePeople(d.writersText).length > 1 ? 's' : ''})`);
    if (directors)
        creators.push(`${directors} (Director${parsePeople(d.directorsText).length > 1 ? 's' : ''})`);
    const date = apaDate(d, true);
...
function apaImage(d) {
    ...
    if (!omitTitle)
        out += `${title}. `;
    if (has(d.publisher || d.siteName))
        out += `${esc(clean(d.publisher || d.siteName))}. `;
    if (has(d.url))
        out += clean(d.url);
    return noFinalPeriodAfterUrl(out);
}
function apaCourseMaterial(d, defaultFormat, includeFullDate = false) {
    ...
    if (!omitTitle)
        out += `${title}. `;
    if (has(d.platform || d.publisher))
        out += `${esc(clean(d.platform || d.publisher))}. `;
    if (has(d.url))
        out += clean(d.url);
    return noFinalPeriodAfterUrl(out);
}
function apaThesis(d) {
    const date = apaDate(d, false);
    ...
    if (!omitTitle)
        out += `${title}. `;
    if (has(d.repository))
        out += `${esc(clean(d.repository))}. `;
    if (has(d.url))
        out += clean(d.url);
    return noFinalPeriodAfterUrl(out);
}
function apaLegalAct(d) {
...
function apaLegalCase(d) {
    ...
    if (legalParts.length)
        out += ` ${esc(legalParts.join(' '))}`;
    out += '.';
    if (has(d.url))
        out += ` ${clean(d.url)}`;
    return noFinalPeriodAfterUrl(out);
}
function apaPersonalCommunication(d) {
...
function apaAiChat(d) {
    ...
    if (has(d.toolName || d.platform))
        out += `${esc(clean(d.toolName || d.platform))}. `;
    if (has(d.url))
        out += clean(d.url);
    else if (has(d.appendix))
        out += `${esc(clean(d.appendix))}`;
    return noFinalPeriodAfterUrl(out);
}
const apaDispatch = {
    webpage: apaWebpage,
 ...
 * ============================================================ */
function apaInTextParenthetical(d, source) {
    if (source === 'legal-act') {
    ...
    if (source === 'legal-case')
        return `(${ital(esc(clean(d.title) || 'Case title'))}${clean(d.year) ? `, ${esc(clean(d.year))}` : ''})`;
...
function apaInTextNarrative(d, source) {
    if (source === 'personal-communication')
        return apaInTextParenthetical(d, source);
    const year = clean(d.year) || 'n.d.';
    const author = resolveInTextAuthor(d, true);
    if (author)
        return `${esc(author)} (${esc(year)})`;
    return `${noAuthorInTextTitle(d, source)} (${esc(year)})`;
}
function apaInTextQuote(d, source) {
    if (source === 'personal-communication')
        return apaInTextParenthetical(d, source);
    if (source === 'legal-act')
        return apaInTextParenthetical(d, source);
...
export function generateApa7(source, data) {
    return {
